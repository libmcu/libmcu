...
TEST(MemoryKVStore, write_ShouldReturnSizeOfWritten) {
	...
	CHECK_EQUAL(sizeof(v8), kvstore_write(storage, "v8", &v8, sizeof(v8)));
	CHECK_EQUAL(sizeof(v16), kvstore_write(storage, "v16", &v16, sizeof(v16)));
	CHECK_EQUAL(sizeof(v32), kvstore_write(storage, "v32", &v32, sizeof(v32)));
	CHECK_EQUAL(sizeof(v64), kvstore_write(storage, "v64", &v64, sizeof(v64)));
	CHECK_EQUAL(sizeof(s), kvstore_write(storage, "str", s, sizeof(s)));
}
...
TEST(MemoryKVStore, write_ShouldOverwrite_WhenTheSameKeyProvided) {
	...
	CHECK_EQUAL(val_overwritten, val_read);
}
...
TEST(MemoryKVStore, read_ShouldReturnTheExactValueWritten) {
	...
	kvstore_write(storage, "key", &val_written, sizeof(val_written));
	kvstore_read(storage, "key", &val_read, sizeof(val_read));
	CHECK_EQUAL(val_written, val_read);
}

TEST(MemoryKVStore, read_ShouldReturnZero_WhenNoEntryForTheKey) {
	uint8_t val_read[80];
	CHECK_EQUAL(0, kvstore_read(storage, "key", &val_read, sizeof(val_read)));
}
...
TEST(MemoryKVStore, new_ShouldKeepDataSeperatedFromOtherNamespaces_WhenNewNamespaceCreated) {
	...
	CHECK_EQUAL(val1_written, val1_read);
	kvstore_read(ns2, "samekey", &val2_read, sizeof(val2_read));
	CHECK_EQUAL(val2_written, val2_read);
