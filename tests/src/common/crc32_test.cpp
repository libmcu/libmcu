...
TEST(CRC32, ShouldComputeJamcrcCRC) {
	const uint8_t t[] = "123456789";
	LONGS_EQUAL(0x340BC6D9, crc32_jamcrc(t, sizeof(t)-1));
}
TEST(CRC32, ShouldComputeCksumCRC) {
	const uint8_t t[] = "123456789";
	LONGS_EQUAL(0x765E7680, crc32_cksum(t, sizeof(t)-1));
}
TEST(CRC32, ShouldGenerateJamcrcTable) {
	...
	uint32_t actual[256];
	crc32_generate_reverse_table(0xEDB88320, actual);
	MEMCMP_EQUAL(expected, actual, sizeof(expected));
}
TEST(CRC32, ShouldGenerateCksumTable) {
	const uint32_t expected[] = {
	...
	uint32_t actual[256];
	crc32_generate_table(0x04C11DB7, actual);
	MEMCMP_EQUAL(expected, actual, sizeof(expected));
}
